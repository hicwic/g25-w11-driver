...
- **Monorepo.** The bridge moves into `g25-driver/virtual-g29/`. The standalone
  `g25-virtual-g29` repo is archived. One CI pipeline, one version number,
...
- **Elevation via an on-demand Windows service.** The tray runs per-user and
  cannot elevate silently. A service (`g25vg29`, `start=demand`) does the
...
flowchart LR
    ...
    subgraph Service (LocalSystem, on-demand)
        S[g25vg29 service] --> B[bridge worker]
    end
...
- Copy the bridge tree into `virtual-g29/` (`src/`, `profiles/`, `scripts/`,
...
- Root `.gitignore`: add `virtual-g29/**/bin/`, `obj/`, `virtual-g29/artifacts/`.
- Archive `g25-virtual-g29` on GitHub; leave a README pointer.
...
- `virtual-g29/src/.../Libg25.cs`: `LibraryImport` over `libg25.dll`.
...
### Phase 3 - `g25vg29` service (done, needs an elevated test)
- `virtual-g29/src/G25VirtualG29Service/` - .NET Worker Service (`g25vg29.exe`,
...
- `src/tray/virtual_g29.{h,cpp}` (`g25_virtual_g29` lib): `presence()` (is the
  `g25vg29` service registered?), `status()` (SCM state + a read of
  `\\.\pipe\g25vg29`), `start()` / `stop()` via the SCM.
- `main.cpp`: when the service is present, `show_menu` adds a **Virtual G29**
  submenu - status line + a checked/unchecked **Virtual G29 mode** item
  that toggles the service. Absent -> no item.
- Verified from an **unprivileged** `virtual_g29_probe`: `start()` / `stop()`
  succeed (the `IU` SDDL grant works), `status()` reads the pipe, the worker and
  virtual G29 are gone after stop.
- Still TODO: "Install Virtual G29..." entry when the payload is present
...
- New `installer/g25-virtual-g29.iss` - **elevated** (`PrivilegesRequired=admin`),
  own AppId, installs to `{autopf}\G25 Virtual G29`:
  - `[Files]` the `dotnet publish` payload (`G25_VG29_PAYLOAD_DIR`).
  - `[Run]` `g25-virtual-g29.exe install-driver` then `g25vg29.exe
    install`.
  - `[UninstallRun]` `g25vg29.exe uninstall` + `g25-virtual-g29.exe
    cleanup`.
  - `[Code]` advises about G HUB if `{commonpf}\LGHUB` exists.
- New bridge verb `install-driver` (install/refresh HIDMaestro, then exit).
- The tray shows no Virtual G29 item until the service is registered (user's choice) -
...
- `release.yml`: `actions/setup-dotnet@v4` (10.x); after the C++ build, publish
  the bridge + service self-contained into `dist/virtual-g29` (with
  `-p:Libg25Dll=dist/stage-x64/bin/libg25.dll`), `ISCC` both `.iss` files,
  attach `g25-virtual-g29-<ver>-setup.exe` to the release.
